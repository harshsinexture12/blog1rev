...
def profile():
    '''
    this functn will help the user to watch his profile &
    he can also upadate the profile details and profile pic 
    using this function
    '''
    if g.email: 
        try:
            ...
            if request.method == 'POST':
                ...
                #print('***************************  ')
                flash("Your profile details upadted successfully ")
                if filename != '':
                    #print('inside ************** ')
                    config_obj = Config()
                    ...
                    #print('***************************  ')
                    flash("Your PROFILE PIC updated successfully ")
                db.session.commit()
...
def dashboard():
    '''
    when the login of the user is once autheticated using registered credentials
...
def sign_up():
    '''
    if the user is not registered on the flask blog site he will need to make a registeration 
    on teh site by adding the correct details and satisfying other validations. Once all the 
    validation are passes successfully he will be registered on the flask blog and redirected to
    login page 
    '''
...
def log_in():
    '''
    If the user already exist onto the app he can make a login using the correct details he used while
    registering on the app
    This functn will mainly authenticate the user using his credentials
    '''
    message = None 
    form = LoginForm()
    if request.method == 'POST':
...
def log_out():
    '''
    using this functn the user can log out of the app if he is already signed in!
    '''
    session['email'] = None 
    return redirect(url_for('user.log_in'))
